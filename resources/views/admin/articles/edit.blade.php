...
@section('header_title', 'แก้ไขบทความ')

@section('content')
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">แก้ไขข้อมูลบทความ</h2>
            <p class="text-sm text-gray-600">แก้ไขข้อมูลบทความ ID: {{ $article->id }}</p>
        </div>
        ...
        <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">รูปภาพปกปัจจุบัน</label>
                    <div class="mt-1 mb-3">
                        <img src="{{ asset('storage/' . $article->thumbnail) }}" alt="{{ $article->title }}" class="h-40 object-cover rounded-md">
                    </div>
                    
                    <label for="thumbnail" class="block text-sm font-medium text-gray-700 mb-1">อัพโหลดรูปภาพใหม่ (ถ้าต้องการเปลี่ยน)</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="content" name="content" rows="12" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('content', $article->content) }}</textarea>
