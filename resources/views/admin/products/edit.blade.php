...
@section('header_title', 'แก้ไขสินค้า')

@section('content')
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800">แก้ไขข้อมูลสินค้า</h2>
            <p class="text-sm text-gray-600">แก้ไขข้อมูลสินค้า ID: {{ $product->id }}</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select id="platform" name="platform" required
                        ...
                        <option value="shopee" {{ old('platform', $product->platform) == 'shopee' ? 'selected' : '' }}>Shopee</option>
                        <option value="lazada" {{ old('platform', $product->platform) == 'lazada' ? 'selected' : '' }}>Lazada</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="url" id="affiliate_url" name="affiliate_url" value="{{ old('affiliate_url', $product->affiliate_url) }}" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                
                <!-- รูปภาพสินค้า -->
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">รูปภาพสินค้าปัจจุบัน</label>
                    <div class="mt-1 mb-3">
                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="h-40 object-cover rounded-md">
                    </div>
                    
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-1">อัพโหลดรูปภาพใหม่ (ถ้าต้องการเปลี่ยน)</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $product->description) }}</textarea>
                </div>
